...
(function () {
  ...
  function initRoot(root) {
    ...
    const pageMode = root.getAttribute('data-if-cfg-page-mode') === 'true';
    ...
    if (!productData || !productData.id) {
      const catalogHandles = Object.keys(catalog);
      if (catalogHandles.length) {
        const bootEntry = catalog[catalogHandles[0]];
        if (bootEntry && bootEntry.product) {
          productData = bootEntry.product;
          if (!productData.handle && bootEntry.handle) productData.handle = bootEntry.handle;
          if (!productData.title && bootEntry.title) productData.title = bootEntry.title;
          if (bootEntry.tags) productData.tags = bootEntry.tags;
          if (bootEntry.colors && bootEntry.colors.length) {
            colors = buildColors(bootEntry.colors);
          }
        }
      }
    }
